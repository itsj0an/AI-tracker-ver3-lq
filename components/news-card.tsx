...
const typeColors: Record<string, string> = {
  ...
  "企业方案": "bg-chart-4/20 text-chart-4 border-chart-4/30",
  "开源项目": "bg-chart-3/20 text-chart-3 border-chart-3/30",
}
...
export function NewsCard({ item }: NewsCardProps) {
  const hasTitle = item.title && item.title !== "无标题"
  const hasSummary = item.summary && item.summary.trim().length > 0
  const hasTags = item.tags && item.tags.length > 0
  const hasTrackingValue = item.trackingValue && item.trackingValue.trim().length > 0
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-start justify-between gap-4">
            <h3 className="text-lg font-semibold leading-tight text-foreground">
              {hasTitle ? item.title : `${item.publisher} 动态更新`}
            </h3>
            <Badge 
              variant="outline" 
              className={`shrink-0 ${typeColors[item.type] || typeColors["功能更新"]}`}
            >
              {item.type}
            </Badge>
          </div>
